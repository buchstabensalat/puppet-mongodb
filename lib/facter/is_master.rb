...
def get_options_from_hash_config(config)
  ...
  result << "--tls --host #{Facter.value(:fqdn)}" if config['net.tls.mode'] == 'requireTLS' || !config['net.tls.PEMKeyFile'].nil? || !config['net.tls.CAFile'].nil?
  result << "--tlsCertificateKeyFile #{config['net.tls.certificateKeyFile']}" unless config['net.tls.icertificateKeyFile'].nil?
